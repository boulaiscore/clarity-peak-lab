...
Privacy-safe native connectors live in `ios-plugin/` and `android-plugin/`.
Calendar connectors must query event times only and return daily aggregates;
event titles, notes, attendees, locations, URLs and calendar names are forbidden.
...
- For passive mobile-context changes, run `npm run test:coach` and preserve the aggregate-only privacy contracts documented in the native plugin READMEs.
