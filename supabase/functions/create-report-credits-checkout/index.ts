...
serve(async (req) => {
  ...
  try {
    ...
    let reportPrice = prices.data.find((p: Stripe.Price) =>
      p.unit_amount === selectedPackage.price && p.currency === 'eur' && !p.recurring
    );
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: successUrl || `${origin}/app/report?payment=success&credits=${selectedPackage.credits}`,
