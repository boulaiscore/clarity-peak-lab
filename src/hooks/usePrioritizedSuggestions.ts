  ...
  const suggestions = useMemo((): Suggestion[] => {
    ...
    if (recoveryEffective >= 45 && totalProgress < 50 && xpRemaining > 0) {
      result.push({
        ...
        body: `${xpRemaining} XP behind — ~${gamesNeeded} drill${gamesNeeded > 1 ? 's' : ''} to get back on track.`,
    ...
    if (recoveryEffective >= 45 && totalProgress >= 50 && totalProgress < 80 && xpRemaining > 0) {
      result.push({
        ...
        body: `${xpRemaining} XP left — ~${gamesNeeded} drill${gamesNeeded > 1 ? 's' : ''} to hit target.`,
        action: "Start Training",
        route: "/neuro-lab",
        icon: Dumbbell,
        colorClass: NEUTRAL_STYLE,
    ...
    if (recoveryEffective >= 70 && totalProgress < 100 && xpRemaining > 0) {
      result.push({
        id: "s2_opportunity",
        priority: 6,
        headline: "High recovery: S2 drills unlocked",
