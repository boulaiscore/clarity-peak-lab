...
export function LoomaTrainingLoop() {
  ...
  const ROTATION_DURATION = 13200; // ~13.2 seconds per full rotation (10% slower)
  ...
  return <div className="py-6 pl-12 pr-4 flex flex-col items-center overflow-visible ml-1">
      <div className="relative w-48 h-48 flex items-center justify-center">
        ...
        rotate: rotation
          ...
          <svg className="w-full h-full" viewBox="0 0 100 100" aria-hidden="true">
            ...
            <circle cx={cx} cy={cy} r={r} fill="none" stroke="currentColor" strokeWidth="0.5" className="text-muted/20" strokeDasharray="3 3" />
        ...
        {/* Train icon - at 75° */}
        <motion.div className="absolute z-10 -translate-x-1/2 -translate-y-1/2" style={polarToPercent(cx, cy, r, 75)}>
          <div className="relative">
            <motion.div className="w-9 h-9 rounded-full bg-background flex items-center justify-center" style={{
            borderWidth: 2,
            borderStyle: "solid",
            borderColor: useTransform(activeIcon, icon => icon === "train" ? "hsl(210, 100%, 60%)" : "hsl(210, 40%, 50%)"),
            boxShadow: useTransform(activeIcon, icon => icon === "train" ? "0 0 20px rgba(59, 130, 246, 0.5)" : "0 0 10px rgba(59, 130, 246, 0.1)")
          }}>
              <motion.div style={{
              color: useTransform(activeIcon, icon => icon === "train" ? "hsl(210, 100%, 60%)" : "hsl(210, 40%, 50%)")
            }}>
                <Dumbbell className="w-4 h-4" />
              </motion.div>
            </motion.div>
            <motion.span className="absolute left-1/2 top-full mt-1 -translate-x-1/2 text-[7px] font-bold uppercase tracking-wider whitespace-nowrap text-center" style={{
            color: useTransform(activeIcon, icon => icon === "train" ? "hsl(210, 100%, 60%)" : "hsl(210, 40%, 50%)")
          }}>
              Train
              <motion.span className="block font-normal normal-case tracking-normal text-[8px] mt-0.5" style={{
              color: useTransform(activeIcon, icon => icon === "train" ? "hsl(210, 100%, 50%)" : "hsl(210, 40%, 40%)")
            }}>
      ...
      <p className="text-[10px] text-muted-foreground/60 text-center mt-12 uppercase tracking-wider w-full">The LOOMA Loop</p>
    </div>;
}