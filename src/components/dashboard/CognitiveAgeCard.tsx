...
export function CognitiveAgeCard() {
  const { data, isLoading, hasWeeklyData } = useCognitiveAge();
  ...
  return (
    <div className="py-2 space-y-4">
      ...
      {isOlderThanChrono && !data.isCalibrating && (
        <motion.div
          initial={{ opacity: 0, y: 4 }}
          animate={{ opacity: 1, y: 0 }}
          className="mx-2 p-3 rounded-xl bg-amber-500/5 border border-amber-500/20"
      ...
      {(data.regressionRisk === "medium" || data.regressionRisk === "high") && (
        <motion.div
          initial={{ opacity: 0, y: 4 }}
          animate={{ opacity: 1, y: 0 }}
          className={cn(
            "mx-2 p-3 rounded-xl border",
            data.regressionRisk === "high"
        ...
        >
          <div className="flex items-start gap-2">
            <TrendingDown className={cn(
              ...
              data.regressionRisk === "high" ? "text-red-500" : "text-amber-500"
            )} />
            <div className="flex-1">
              ...
              <Link 
                to="/app/training"
                className={cn(
                  "inline-flex items-center gap-1 mt-2 text-[10px] font-medium transition-colors",
                  data.regressionRisk === "high"
      ...
      {hasWeeklyData && data.baselineScore90d !== null && (
        <BaselineVisual
          ...
          score30d={data.score30d}
          score90d={data.score90d}
        />
      )}

      {/* Next update countdown */}
      {data.daysUntilNextUpdate !== null && (
...
function BaselineVisual({ baselineScore, score30d, score90d }: BaselineVisualProps) {
  const trend = useMemo(() => {
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 h-1.5 bg-muted/50 rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: `${Math.min(100, score90d)}%` }}
              />
            </div>
            <span className="text-[10px] font-medium text-foreground w-8 text-right">
              {Math.round(score90d)}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 h-1.5 bg-muted/50 rounded-full overflow-hidden">
              <div 
                className={cn(
                  "h-full rounded-full",
                  score30d >= (score90d ?? baselineScore)
                    ? "bg-emerald-500/60"
                    : "bg-amber-500/60"
                )}
                style={{ width: `${Math.min(100, score30d)}%` }}
              />
            </div>
            <span className="text-[10px] font-medium text-foreground w-8 text-right">
              {Math.round(score30d)}
