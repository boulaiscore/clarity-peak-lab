...
export function ReasoningQualityCard({
  ...
  if (isLoading) {
    return <div className="p-4 rounded-xl bg-card border border-border/40 animate-pulse">
  ...
  return <motion.button initial={{
    ...
    duration: 0.3,
    ease: "easeOut"
  }} onClick={() => navigate("/app/reasoning-quality-impact")} className="w-full p-4 rounded-xl bg-card border border-border/40 text-left hover:bg-card/80 transition-colors">
      {/* Header row */}
      ...
      <div className="flex items-baseline gap-2 mb-3">
        ...
        {deltaVsYesterday && <span className="text-[10px] font-medium tabular-nums ml-1 text-primary">
            {deltaVsYesterday}
          </span>}
      </div>
